...
export default function ThreeDAnimationsPage() {
  ...
  const update3DAnimationsSection = useMutation(
    api.threeDAnimations.update3DAnimationsSection
  );
  const create3DAnimationProject = useMutation(
    api.threeDAnimations.create3DAnimationProject
  );
  const update3DAnimationProject = useMutation(
    api.threeDAnimations.update3DAnimationProject
  );
  const delete3DAnimationProject = useMutation(
    api.threeDAnimations.delete3DAnimationProject
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await update3DAnimationsSection({
      ...
      toast.success("3D Animations section updated", {
        description: "Your changes have been saved",
      });
    } catch (error) {
      console.error("Error updating 3D Animations section:", error);
      toast.error("Error updating 3D Animations section", {
  ...
  const uploadImage = async (
    file: File,
    projectId?: Id<"threeDAnimationsProjects">
  ): Promise<string> => {
    ...
    const { url } = await saveImage({
      storageId,
      destination: "threeDAnimationProject",
      projectId: projectId as
        | Id<"threeDAnimationsProjects">
        | Id<"twoDAnimationsProjects">
        | undefined,
    });
  ...
  const openEditProjectDialog = (project: {
    _id: Id<"threeDAnimationsProjects">;
    title: string;
    description: string;
    thumbnailUrl: string;
    videoUrl: string;
  ...
  const handleProjectSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && currentProjectId) {
        await update3DAnimationProject({
          id: currentProjectId,
        ...
        await create3DAnimationProject({
          title: projectFormData.title,
          description: projectFormData.description,
          thumbnailUrl: finalThumbnailUrl,
          videoUrl: projectFormData.videoUrl,
  ...
  const openDeleteDialog = (id: Id<"threeDAnimationsProjects">) => {
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      await delete3DAnimationProject({ id: projectToDelete });
      ...
      <Card className="border-l-0 border-b-0 border-r-0 shadow-none">
        <CardHeader>
          {animationSectionContent === undefined ? (
            ...
            <>
              <CardTitle>3D Animations Section Management</CardTitle>
              <CardDescription>
                Update the content of your 3D animations section that appears on
                your homepage.
              </CardDescription>
            </>
          )}
        ...
        <CardHeader className="flex flex-col sm:flex-row sm:items-center gap-5 justify-between">
          <div>
            {animationProjects === undefined ? (
              ...
              <>
                <CardTitle>3D Animation Projects</CardTitle>
                <CardDescription>
                  Manage your 3D animation projects. Only the 3 most recent
          ...
          {animationProjects === undefined ? (
            ...
            <div className="flex items-center flex-col gap-5 py-8">
              ...
              <p className="text-muted-foreground">
                No 3D animation projects yet. Create your first one!
              </p>
            </div>
          ) : (
            <Table>
      ...
      <Dialog open={isProjectDialogOpen} onOpenChange={setIsProjectDialogOpen}>
        <DialogContent className="sm:max-w-[600px] max-h-[80vh] sm:max-h-[85vh] md:max-h-[90vh] overflow-auto bg-[#111111]">
          <DialogHeader>
            <DialogTitle>
              {isEditMode
                ? "Edit 3D Animation Project"
                : "Create New 3D Animation Project"}
            </DialogTitle>
            <DialogDescription>
              {isEditMode
                ? "Update the details of your 3D animation project."
                : "Add a new 3D animation project to your portfolio."}
            </DialogDescription>
          </DialogHeader>
