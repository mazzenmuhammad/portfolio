...
export default function VideoEditingPage() {
  ...
  const updateVideoSection = useMutation(api.video.updateVideoSection);
  const createVideoProject = useMutation(api.video.createVideoProject);
  const updateVideoProject = useMutation(api.video.updateVideoProject);
  const deleteVideoProject = useMutation(api.video.deleteVideoProject);
  ...
  useEffect(() => {
    if (videoSectionContent) {
      setFormData({
        title: videoSectionContent.title,
        description: videoSectionContent.description,
      });
      setOriginalData({
        title: videoSectionContent.title,
        description: videoSectionContent.description,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Error updating video section", {
  ...
  const openDeleteDialog = (id: Id<"videoProjects">) => {
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      const project = videoProjects?.find((p) => p._id === projectToDelete);
      ...
      await deleteVideoProject({ id: projectToDelete });
    ...
    <div className="space-y-6 w-full bg-card">
      <Card className="border-l-0 border-b-0 border-r-0 shadow-none">
        <CardHeader>
          {videoSectionContent === undefined ? (
            ...
            <>
              <CardTitle>Video Editing Section Management</CardTitle>
              <CardDescription>
                Update the content of your video section that appears on your
                homepage.
              </CardDescription>
            </>
          )}
        ...
        <CardHeader className="flex flex-col sm:flex-row sm:items-center gap-5 justify-between">
          <div>
            {videoProjects === undefined ? (
              ...
              <>
                <CardTitle>Video Editing Projects</CardTitle>
                <CardDescription>
                  Manage your video projects. Only the 3 most recent projects
          ...
          {videoProjects === undefined ? (
            ...
            <div className="flex items-center flex-col gap-5 py-8">
              ...
              <p className="text-muted-foreground">
                No video projects yet. Create your first one!
              </p>
            </div>
          ) : (
            <Table>
      ...
      <Dialog open={isProjectDialogOpen} onOpenChange={setIsProjectDialogOpen}>
        <DialogContent className="sm:max-w-[600px] max-h-[80vh] sm:max-h-[85vh] md:max-h-[90vh] overflow-auto bg-[#111111]">
          <DialogHeader>
            <DialogTitle>
              {isEditMode ? "Edit Video Project" : "Create New Video Project"}
            </DialogTitle>
            <DialogDescription>
              {isEditMode
                ? "Update the details of your video project."
                : "Add a new video project to your portfolio."}
            </DialogDescription>
          </DialogHeader>
